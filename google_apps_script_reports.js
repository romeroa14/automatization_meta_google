/**
 * Google Apps Script para generar reportes estadísticos en Google Slides
 * Este script maneja la creación de presentaciones y diapositivas automáticamente
 */
...
/**
 * Función para manejar solicitudes GET (para pruebas)
 */
function doGet(e) {
...
function handleCreatePresentation(data) {
  try {
    ...
    // Nota: Google Slides no tiene setDescription, usamos el título como descripción
    console.log('Descripción configurada:', description);
    
    // Obtener la primera diapositiva y configurarla como portada
    const slides = presentation.getSlides();
    if (slides.length > 0) {
      const firstSlide = slides[0];
      firstSlide.getLayout().setLayoutName('TITLE_AND_SUBTITLE');
      
      // Configurar título y subtítulo
      const titleShape = firstSlide.getShapes()[0];
      if (titleShape) {
        titleShape.getText().setText(title);
      }
      
      const subtitleShape = firstSlide.getShapes()[1];
      if (subtitleShape) {
...
function handleCreateSlide(data) {
  try {
    ...
    const layoutName = SLIDE_LAYOUTS[slideData.layout] || 'TITLE_AND_BODY';
    slide.getLayout().setLayoutName(layoutName);
...
/**
 * Crea una diapositiva de portada
 */
function createCoverSlide(slide, data) {
  ...
  if (data.content && shapes.length > 2) {
    ...
    contentText += `Marcas: ${data.content.total_brands}\n`;
...
/**
 * Crea una diapositiva de título de marca
 */
...
/**
 * Crea una diapositiva de campaña
 */
...
/**
 * Crea una diapositiva de gráfica
 */
...
/**
 * Crea una diapositiva de resumen
 */
function createSummarySlide(slide, data) {
  const shapes = slide.getShapes();
  ...
  if (data.content && shapes.length > 2) {
    ...
    contentText += `📊 CPM Promedio: ${data.content.average_cpm}\n`;
    contentText += `🎯 CPC Promedio: ${data.content.average_cpc}\n\n`;
    contentText += `🕐 Generado: ${data.content.generated_at}`;
