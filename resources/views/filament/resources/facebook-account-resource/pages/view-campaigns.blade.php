                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            @if($ad['creative'] && ($ad['creative']['local_image_path'] || $ad['creative']['thumbnail_url']))
                                                <div class="flex-shrink-0 h-20 w-32 mr-4 relative group">
                                                    @php
                                                        $imageUrl = $ad['creative']['local_image_path'] 
                                                            ? asset($ad['creative']['local_image_path']) 
                                                            : $ad['creative']['thumbnail_url'];
                                                    @endphp
                                                    <img class="h-20 w-32 rounded-lg object-cover border-2 border-gray-200 shadow-sm hover:shadow-md transition-shadow duration-200" 
                                                         src="{{ $imageUrl }}" 
                                                         alt="Anuncio {{ $ad['ad_name'] }}"
                                                         loading="lazy"
                                                         onerror="this.parentElement.innerHTML='<div class=\'h-20 w-32 bg-gray-200 rounded-lg flex items-center justify-center border-2 border-gray-300\'><svg class=\'h-8 w-8 text-gray-400\' fill=\'none\' stroke=\'currentColor\' viewBox=\'0 0 24 24\'><path stroke-linecap=\'round\' stroke-linejoin=\'round\' stroke-width=\'2\' d=\'M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 002 2v12a2 2 0 002 2z\'></path></svg></div>';">
                                                    
                                                    <!-- Indicador de imagen local -->
                                                    @if($ad['creative']['local_image_path'])
                                                        <div class="absolute -top-2 -right-2 bg-green-500 text-white text-xs rounded-full w-5 h-5 flex items-center justify-center shadow-sm" title="Imagen descargada localmente">
                                                            ✓
                                                        </div>
                                                    @endif
                                                    
                                                    <!-- Tooltip con información del creative -->
                                                    <div class="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-50 transition-all duration-200 rounded-lg flex items-center justify-center opacity-0 group-hover:opacity-100">
                                                        <div class="text-white text-xs text-center px-2">
                                                            <div class="font-semibold">Ver imagen</div>
                                                            <div class="text-xs opacity-75">{{ $ad['creative']['title'] ?? 'Sin título' }}</div>
                                                        </div>
                                                    </div>
                                                </div>
                                            @else
